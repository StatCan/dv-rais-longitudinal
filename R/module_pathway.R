...
pathway_server <- function(id, language) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    resp <-
      GET(
        "https://github.com/StatCan/dv-rais-longitudinal/raw/master/data/pathway.Rds",
        write_disk(tmp)
      )
    ...
    output$outBarChart <- renderPlotly({
      ...
      cert_text <- format_pct(df()$cert, locale=language())
      cont_text <- format_pct(df()$cont, locale=language())
      disc_text <- format_pct(df()$disc, locale=language())
